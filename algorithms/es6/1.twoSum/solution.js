 ...
 */
export function brutal(nums, target) {
 ...
 */
export function hash(nums, target) {
    if (!(Array.isArray(nums) && nums.length > 1) || typeof target !== "number") {
        return;
    }
 ...
 */
export let extend = {}; // extend is for inspired solutions for similar problem, it doesn't solve the problem directly so may move it elsewhere in future.
